...
Economists measure the effect with the Herfindahl-Hirschman Index, the sum of the squared shares, and the index converts into the figure that matters: one divided by it gives the effective denominator, the number of equal-sized units the base actually behaves like. Twelve customers with the largest at 40 percent of revenue behave like slightly more than five. You have twelve customers. For risk purposes, you have five. The recount nearly always comes back smaller, and it explains how a business can clear the Rule of 30 while remaining thin: fifty customers with one at 60 percent of revenue pass the counted test and fail the effective one.
...
If the case for this kind of reasoning still feels soft, the twentieth century left a hard one: Allied statisticians estimated German tank production from the serial numbers of a handful of captured tanks at around 270 a month, while conventional intelligence, with its full apparatus of reports and informants, said 1,400. Postwar records showed 276. An existence proof, not a template, since your deals do not carry serial numbers. But it is this chapter's claim made unanswerable: a tiny sample with structure inside it, honestly read, beat the volume machine by a factor of five.
...
The Coverage Index is not a sophisticated model. It is a count, and its thresholds are not physical constants but operating alarms: their job is not to certify safety but to force a visible discussion before one event can dominate the system. For each dimension, mark the current state as Green, Yellow, or Red.
...
In March 2000, lightning started a ten-minute fire in an Albuquerque semiconductor plant that supplied radio-frequency chips to both Nokia and Ericsson; the smoke and the sprinklers contaminated the cleanrooms, and a promised one-week disruption became months. Nokia, which had cultivated second sources, moved within days to reconfigure chips for other suppliers' lines and lock up the world's spare capacity, and its phone production barely stuttered. Ericsson had single-sourced the chips: no second supplier to call, hundreds of millions of dollars of sales lost in a boom market, and within two years an exit from handset manufacturing under its own name. Same fire, same supplier. The entire difference was a purchase Nokia had made years earlier: the redundancy that reads as pure cost on every invoice until the week it is the company. The expected-value calculation that says a backup supplier is waste is correct, and catastrophically incomplete. It is the same calculation behind every buffer that ever got trimmed to make a quarter.
...
*"Bradley Efron and Carl Morris"*: Bradley Efron and Carl Morris, "Stein's Paradox in Statistics," *Scientific American* 236, no. 5 (1977), which works the eighteen-player baseball example; the underlying result is Charles Stein's, from 1955. The formal guarantee is specific (squared-error loss, several quantities estimated together); the managerial reading is directional: thin-sample estimates improve when pulled toward a base rate.
