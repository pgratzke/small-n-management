...
The first task is to understand the boundary itself. And this is where a deeper dive into statistics can help us.

## The Most Dangerous Equation

This book asks you to hold exactly one mathematical relationship in your head, and this is it: uncertainty shrinks only with the square root of the count. Most of what is strange about managing at small n falls out of that one line.
...
The Coverage Index is not a sophisticated model. It is a count, and its thresholds are operating alarms rather than physical constants, there to force a visible discussion before one event can dominate the system. Set the levels to your economics: a business that can replace a customer in six weeks can run hotter than one that needs eighteen months. The alarm levels are adjustable; the obligation to have alarms is not. For each dimension, mark the current state as Green, Yellow, or Red.
